...
export default async function Dashboard(){
  ...
  return <AdminShell utilisateur={u} titre={`Bienvenue à ${e.nom}`} description="Le tableau de bord affiche uniquement les widgets autorisés.">
    ...
    <div className={styles.grilleStats}>{va&&<article className={styles.stat}><span><CalendarDays/></span><div><small>Périodes académiques</small><strong>{annees}</strong></div></article>}{vs&&<article className={styles.stat}><span><BookOpenCheck/></span><div><small>Sections actives</small><strong>{sections}</strong></div></article>}{vc&&<article className={styles.stat}><span><School/></span><div><small>Classes / promotions</small><strong>{classes}</strong></div></article>}{ve&&<article className={styles.stat}><span><UsersRound/></span><div><small>Apprenants inscrits</small><strong>{eleves}</strong></div></article>}</div>
    {vconfig&&<section className={styles.panneau}><div className={styles.panneauEntete}><div><h2>Configuration rapide</h2><p>Seuls les raccourcis autorisés sont affichés.</p></div></div><div className={styles.panneauCorps}><div className={styles.cartesRapides}>{possede(p,"PARAMETRES_VOIR",s)&&<Link href="/dashboard/parametres" className={styles.carteRapide}><span><Settings/></span><h3>Identité de l’établissement</h3><p>Paramètres officiels.</p></Link>}{possede(p,"ANNEES_SCOLAIRES_VOIR",s)&&<Link href="/dashboard/annees-scolaires" className={styles.carteRapide}><span><CalendarDays/></span><h3>Périodes académiques</h3><p>Consulter les périodes.</p></Link>}{possede(p,"SECTIONS_VOIR",s)&&<Link href="/dashboard/sections" className={styles.carteRapide}><span><BookOpenCheck/></span><h3>Sections</h3><p>Consulter les cycles.</p></Link>}{possede(p,"CLASSES_VOIR",s)&&<Link href="/dashboard/classes" className={styles.carteRapide}><span><School/></span><h3>Classes / promotions</h3><p>Consulter les classes et promotions.</p></Link>}</div></div></section>}
  </AdminShell>;
}
