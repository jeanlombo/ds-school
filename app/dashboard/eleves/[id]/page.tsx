...
export default async function ProfilEleve({ params, searchParams }: Props) {
  ...
  return <AdminShell utilisateur={utilisateur} titre="Dossier de l’élève" description="Profil centralisé, inscription, responsables et suivi scolaire." action={<div className={elevesStyles.headerActions}><Link href={`/dashboard/eleves/${eleve.id}/carte`} className={styles.boutonSecondaire}><CreditCard size={18}/> Carte scolaire</Link><Link href={`/dashboard/eleves/${eleve.id}/modifier`} className={styles.boutonPrimaire}><Pencil size={18}/> Modifier</Link><Link href="/dashboard/eleves" className={styles.boutonSecondaire}><ArrowLeft size={18}/> Retour</Link></div>}>
    ...
    <div className={styles.deuxColonnes}>
      <div>
        ...
        <section className={styles.panneau}><div className={styles.panneauEntete}><div><h2>Historique des inscriptions</h2><p>Parcours académique enregistré.</p></div></div><div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Année</th><th>Section</th><th>Classe</th><th>Admission</th><th>Statut</th></tr></thead><tbody>{eleve.inscriptions.map(i=><tr key={i.id}><td>{i.anneeScolaire.libelle}</td><td>{i.classe.section.nom}</td><td><strong>{i.classe.nom}</strong></td><td>{i.typeAdmission}</td><td><span className={styles.badge}>{i.statut}</span></td></tr>)}</tbody></table></div></section>
