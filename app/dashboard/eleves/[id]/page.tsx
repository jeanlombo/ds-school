...
export default async function ProfilEleve({ params, searchParams }: Props) {
  ...
  return <AdminShell utilisateur={utilisateur} titre={`Dossier de l’${t.personne}`} description={`Profil centralisé, inscription, responsables et suivi ${t === undefined ? "académique" : "académique"}.`} action={<div className={elevesStyles.headerActions}><Link href={`/dashboard/eleves/${eleve.id}/carte`} className={styles.boutonSecondaire}><CreditCard size={18}/> {t.carte}</Link><Link href={`/dashboard/eleves/${eleve.id}/modifier`} className={styles.boutonPrimaire}><Pencil size={18}/> Modifier</Link><Link href="/dashboard/eleves" className={styles.boutonSecondaire}><ArrowLeft size={18}/> Retour</Link></div>}>
