...
type ClasseOption = { id: number; nom: string; sectionNom: string; sectionType?: string | null };
...
function sectionSuperieure(nom: string, typeSection?: string | null) {
  const s = normaliser(`${typeSection || ""} ${nom}`.replaceAll("_", " "));
...
export default function FormulaireNouvelApprenant({
  ...
  const classesScolaires = useMemo(
    () => classes.filter(c => !sectionSuperieure(c.sectionNom, c.sectionType)),
    [classes]
  );
