...
import InscriptionApprenantAdaptative from "@/components/eleves/InscriptionApprenantAdaptative";
...
export default async function NouvelEleve({ searchParams }: Props) {
  ...
  return <AdminShell utilisateur={utilisateur} titre="Nouvelle inscription" description="Créez le dossier administratif et académique de l’apprenant. La terminologie s’adapte à la section choisie." action={<Link href="/dashboard/eleves" className={styles.boutonSecondaire}><ArrowLeft size={18}/> Retour à la liste</Link>}>
    ...
    <form action={creerEleve} className={elevesStyles.formulaireLong} encType="multipart/form-data">
      ...
      <InscriptionApprenantAdaptative annees={annees.map(a=>({id:a.id,libelle:a.libelle,active:a.active}))} classes={classes.map(c=>({id:c.id,nom:c.nom,section:{nom:c.section.nom,code:c.section.code}}))} aujourdHui={aujourdHui}/>
