...
export default async function NouvelEleve({ searchParams }: Props) {
  ...
  if (!classes.length || !annees.length) return <AdminShell utilisateur={utilisateur} titre="Nouvelle inscription" description="Le socle académique doit être prêt avant l’inscription."><div className={styles.infoBandeau}>Créez au moins une classe et une année scolaire ouverte avant d’inscrire un apprenant.</div><Link className={styles.boutonPrimaire} href="/dashboard/classes">Configurer les classes</Link></AdminShell>;
  const matricule = `${ecole.code}-${new Date().getFullYear()}-${String(compteur + 1).padStart(5, "0")}`;
  const aujourdHui = new Date().toISOString().slice(0,10);

  return <AdminShell utilisateur={utilisateur} titre="Nouvelle inscription" description="Créez le dossier de l’apprenant. Élève ou Étudiant sera déterminé par sa section." action={<Link href="/dashboard/eleves" className={styles.boutonSecondaire}><ArrowLeft size={18}/> Retour à la liste</Link>}>
    ...
    <form action={creerEleve} className={elevesStyles.formulaireLong} encType="multipart/form-data">
      <section className={styles.panneau}><div className={styles.panneauEntete}><div className={elevesStyles.titreSection}><span><UserRound/></span><div><h2>Informations personnelles</h2><p>Identité officielle de l’apprenant et photo utilisée sur sa carte d’identification.</p></div></div></div><div className={styles.panneauCorps}>
        <PhotoEleveUpload />
      ...
      <section className={styles.panneau}><div className={styles.panneauEntete}><div className={elevesStyles.titreSection}><span><School/></span><div><h2>Informations scolaires / académiques</h2><p>Affectation à la classe ou promotion et à l’année scolaire ou académique.</p></div></div></div><div className={styles.panneauCorps}><div className={styles.formGrille}>
        ...
        <div className={styles.champ}><label>Type d’admission</label><select name="typeAdmission" defaultValue="nouveau"><option value="nouveau">Nouveau / débutant</option><option value="ancien">Ancien apprenant</option><option value="transfert">Transfert</option></select></div>
      ...
      <section className={styles.panneau}><div className={styles.panneauEntete}><div className={elevesStyles.titreSection}><span><UsersRound/></span><div><h2>Responsables / personnes de référence</h2><p>Enregistrez au moins un contact joignable. Cette zone convient aux élèves comme aux étudiants.</p></div></div></div><div className={styles.panneauCorps}>
        <div className={elevesStyles.responsablesGrille}>{["pere","mere","tuteur"].map((type)=><fieldset key={type} className={elevesStyles.responsable}><legend>{type === "pere" ? "Père / Référence 1" : type === "mere" ? "Mère / Référence 2" : "Tuteur / Référence 3"}</legend><label>Nom complet<input name={`${type}Nom`}/></label><label>Téléphone<input name={`${type}Telephone`}/></label><label>E-mail<input type="email" name={`${type}Email`}/></label><label>Profession<input name={`${type}Profession`}/></label><label>Adresse<input name={`${type}Adresse`}/></label></fieldset>)}</div>
        <div className={styles.champ}><label>Contact principal</label><select name="responsablePrincipal" defaultValue="tuteur"><option value="pere">Père</option><option value="mere">Mère</option><option value="tuteur">Tuteur</option></select></div>
      </div></section>
      ...
      <div className={elevesStyles.barreValidation}><span>Les champs marqués d’un astérisque sont obligatoires.</span><BoutonSoumission texte="Enregistrer l’apprenant" icone={<Save size={18}/>}/></div>
    </form>
  </AdminShell>;
}
