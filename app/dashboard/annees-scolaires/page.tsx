...
export default async function Annees({ searchParams }: { searchParams: Promise<{ succes?: string; erreur?: string }> }) {
  ...
  return <AdminShell utilisateur={utilisateur} titre="Années scolaires / académiques" description="Ouvrez, activez et clôturez les périodes académiques.">
    {query.succes && <div className={styles.message}>La période académique a été créée avec succès.</div>}
    ...
    <div className={styles.deuxColonnes}>
      <section className={styles.panneau}><div className={styles.panneauEntete}><div><h2>Historique des périodes</h2><p>{annees.length} période(s) enregistrée(s)</p></div><CalendarCheck size={22}/></div>
        {annees.length ? <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Année / période</th><th>Période</th><th>État</th><th>Actions</th></tr></thead><tbody>{annees.map(a=><tr key={a.id}><td><strong>{a.libelle}</strong></td><td>{dateFR(a.dateDebut)} — {dateFR(a.dateFin)}</td><td><span className={`${styles.badge} ${a.active ? styles.badgeActif : a.statut === "cloturee" ? styles.badgeInactif : ""}`}>{a.active ? "Période active" : a.statut === "cloturee" ? "Clôturée" : "Ouverte"}</span></td><td><div className={styles.actionsTable}>{!a.active && a.statut !== "cloturee" && <form action={activerAnnee}><input type="hidden" name="id" value={a.id}/><button title="Activer"><CheckCircle2 size={16}/></button></form>}<form action={basculerStatutAnnee}><input type="hidden" name="id" value={a.id}/><input type="hidden" name="statut" value={a.statut}/><button title={a.statut === "ouverte" ? "Clôturer" : "Rouvrir"}><LockKeyhole size={16}/></button></form></div></td></tr>)}</tbody></table></div> : <div className={styles.vide}><CalendarDays size={38}/><p>Aucune période scolaire / académique créée.</p></div>}
      </section>
      <section className={styles.panneau}><div className={styles.panneauEntete}><div><h2>"Nouvelle période scolaire / académique"</h2><p>Créez la prochaine période académique.</p></div><CalendarDays size={22}/></div><form action={creerAnnee} className={styles.panneauCorps}><div className={styles.formGrille}><div className={`${styles.champ} ${styles.champLarge}`}><label>Libellé *</label><input name="libelle" placeholder="Ex. 2026-2027" required/></div><div className={styles.champ}><label>Date de début *</label><input type="date" name="dateDebut" required/></div><div className={styles.champ}><label>Date de fin *</label><input type="date" name="dateFin" required/></div></div><div className={styles.actions}><BoutonSoumission texte="Créer la période"/></div></form></section>
    </div>
  </AdminShell>;
}
