...
export default async function Page({params,searchParams}:Props){
 ...
 return <main className={styles.documentPage}>
  ...
  <article className={styles.document}>
   <header className={styles.docHeader}><div className={styles.logo}>{ecole.logo?<img src={ecole.logo} alt="Logo"/>:<span>DS</span>}</div><div><small>RÉPUBLIQUE DÉMOCRATIQUE DU CONGO</small><h1>{ecole.nom}</h1><p>{ecole.slogan||'Excellence · Discipline · Innovation'}</p><h2>RELEVÉ OFFICIEL DE NOTES</h2></div><div className={styles.number}><small>N° DU RELEVÉ</small><strong>{numero}</strong><span>Année {periode.anneeScolaire.libelle}</span></div></header>
   <section className={styles.identity}><div><small>Nom complet</small><strong>{inscription.eleve.nom} {inscription.eleve.postnom??''} {inscription.eleve.prenom}</strong></div><div><small>Matricule</small><strong>{ligne.matricule}</strong></div><div><small>Sexe</small><strong>{inscription.eleve.sexe}</strong></div><div><small>Classe</small><strong>{inscription.classe.nom}</strong></div><div><small>Section</small><strong>{inscription.classe.section.nom}</strong></div><div><small>Période</small><strong>{periode.nom}</strong></div></section>
   <section className={styles.docSection}><div className={styles.sectionTitle}><h3>Résultats académiques</h3><span>{ligne.matieres.length} matière(s)</span></div><table className={styles.docTable}><thead><tr><th>N°</th><th>Matière</th><th>Évaluations</th><th>Moyenne / 100</th><th>Appréciation</th></tr></thead><tbody>{ligne.matieres.map((m,i)=><tr key={m.matiereId}><td>{i+1}</td><td>{m.nom}</td><td>{m.evaluations}</td><td><strong>{m.moyenne.toFixed(2)}</strong></td><td>{m.moyenne>=80?'Excellent':m.moyenne>=70?'Très bien':m.moyenne>=60?'Bien':m.moyenne>=50?'Assez bien':'À renforcer'}</td></tr>)}</tbody></table></section>
