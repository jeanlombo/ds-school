...
export default async function Page({searchParams}:Props){
 ...
 const classe=classes.find(c=>c.id===classeId); const periode=periodes.find(p=>p.id===periodeId);
 return <AdminShell utilisateur={utilisateur} titre="Relevés de notes officiels" description="Générez des relevés académiques sécurisés et imprimables.">
  <div className={styles.page}><RetourDashboard/>
   ...
   <section className={styles.filters}><form><label><span>Classe</span><select name="classeId" defaultValue={classeId}>{classes.map(c=><option key={c.id} value={c.id}>{c.nom} — {c.section.nom}</option>)}</select></label><label><span>Période</span><select name="periodeId" defaultValue={periodeId}>{periodes.map(p=><option key={p.id} value={p.id}>{p.anneeScolaire.libelle} — {p.nom}</option>)}</select></label><label><span>Recherche</span><input name="recherche" defaultValue={q.recherche??''} placeholder="Nom ou matricule"/></label><button><Search size={17}/>Afficher</button></form></section>
   <section className={styles.kpis}><article><Users/><div><small>Apprenants</small><strong>{lignes.length}</strong></div></article><article><ShieldCheck/><div><small>Classe</small><strong>{classe?.nom??'—'}</strong></div></article><article><FileCheck2/><div><small>Période</small><strong>{periode?.nom??'—'}</strong></div></article><article><FileCheck2/><div><small>Relevés disponibles</small><strong>{lignes.length}</strong></div></article></section>
   <section className={styles.panel}><div className={styles.panelHead}><div><span>Liste officielle</span><h3>{classe?.nom??'Classe'} · {periode?.anneeScolaire.libelle??''}</h3></div></div><div className={styles.tableWrap}><table><thead><tr><th>Rang</th><th>Matricule</th><th>Apprenant</th><th>Moyenne</th><th>Mention</th><th>Décision</th><th>Action</th></tr></thead><tbody>{lignes.map(l=><tr key={l.inscriptionId}><td>{l.rang}</td><td>{l.matricule}</td><td><strong>{l.nomComplet}</strong></td><td>{l.moyenne.toFixed(2)}%</td><td>{l.mention}</td><td>{l.decision}</td><td><Link className={styles.open} href={`/dashboard/centre-academique/releves-notes/${l.inscriptionId}?classeId=${classeId}&periodeId=${periodeId}`}>Ouvrir le relevé</Link></td></tr>)}</tbody></table>{lignes.length===0&&<div className={styles.empty}>Aucun apprenant trouvé pour cette sélection.</div>}</div></section>
  </div>
 </AdminShell>
}
