...
export async function creerEleve(formData: FormData) {
  ...
  const sectionNormalisee = classe.section.nom
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase();

  const personne =
    sectionNormalisee.includes("univers") ||
    sectionNormalisee.includes("institut superieur")
      ? "Étudiant"
      : "Élève";

  redirect(
    `/dashboard/eleves/${eleve.id}?succes=${encodeURIComponent(
      `${personne} enregistré avec succès`
    )}`
  );
}
